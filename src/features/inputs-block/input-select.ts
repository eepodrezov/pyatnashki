...
function getInputSelect() {
    const options = validOptionsArray.reduce((acc,option) => {
        const isSelected = localStorage.getItem('selectedValue') === option.value
        return acc + `<option ${isSelected && 'selected'} value="${option.value}">${option.value}</option>`
    },'')
    return `
        <form>
            <label for="inputOptions">Варианты заполнения</label>

            <select name="inputOptions" id="inputOptions">
                <option value="">Выберите вариант</option>
                ${options}
            </select>
            <button id="selectSubmit">run</button>
        </form>
    `
}

export const InputSelect = getInputSelect()